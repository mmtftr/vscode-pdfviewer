...
const DELETE_GRACE_PERIOD_MS = 1000;
const MISSING_POLL_INTERVAL_MS = 1000;

export class PdfPreview extends Disposable {
  private _previewState: PreviewState = 'Visible';
  private _pendingDelete: ReturnType<typeof setTimeout> | undefined;
  private _reloadedVersion: string | undefined;
  private _missingPoll: ReturnType<typeof setInterval> | undefined;
  private _watchers: vscode.Disposable | undefined;
        ...
        switch (message.type) {
          ...
          case 'position': {
            if (typeof message.position === 'string') {
              positions.set(resource, message.position);
            }
            break;
          }
        }
  ...
  private watchResource(): void {
    ...
    this._watchers = { dispose: () => disposeAll(disposables) };

    // Watch the containing folder rather than a glob of the file path: a
    // RelativePattern also works outside the workspace, and '*' sidesteps
    // glob characters in the file name.
    const resourceRoot = vscode.Uri.joinPath(this.resource, '..');
    const watcher = vscode.workspace.createFileSystemWatcher(
      new vscode.RelativePattern(resourceRoot, '*')
    );
    const onEvent = (e: vscode.Uri): void => {
      if (e.toString() === this.resource.toString()) {
        this.onResourceEvent();
      }
    };
    disposables.push(
      watcher,
    ...
    // VS Code batches watcher events for a few hundred milliseconds; watch
    // local files directly as well so rebuilt PDFs show up right away.
    if (this.resource.scheme === 'file') {
      const dir = path.dirname(this.resource.fsPath);
      // Removing the folder is reported under the folder's own name
      const names = [path.basename(this.resource.fsPath), path.basename(dir)];
      try {
        const fsWatcher = fs.watch(dir, (_event, filename) => {
          if (!filename || names.includes(filename.toString())) {
            this.onResourceEvent();
          }
        });
        fsWatcher.on('error', () => fsWatcher.close());
        disposables.push({ dispose: () => fsWatcher.close() });
      } catch {
        // The VS Code watcher still covers this file.
      }
    }
  }

  private async onResourceEvent(): Promise<void> {
    let stat: { mtime: number; size: number };
    try {
      stat = await this.statResource();
    } catch {
      this.onResourceDeleted();
      return;
    }
    if (this._missingPoll) {
      // The watchers may have died with the file's folder
      this.watchResource();
    }
    this.clearMissingState();
    // Both watchers report most changes; reload once per version of the file.
    const version = `${stat.mtime}:${stat.size}`;
    if (version !== this._reloadedVersion) {
      this._reloadedVersion = version;
      this.postToWebview('reload');
    }
  }

  private async statResource(): Promise<{ mtime: number; size: number }> {
    if (this.resource.scheme === 'file') {
      // Skips the round trip through VS Code's file service
      const { mtimeMs, size } = await fs.promises.stat(this.resource.fsPath);
      return { mtime: mtimeMs, size };
    }
    return vscode.workspace.fs.stat(this.resource);
  }

  private onResourceDeleted(): void {
    // Build tools often replace the output by deleting and recreating it, so
    // only report the file as missing if it stays gone.
    if (this._pendingDelete || this._missingPoll) {
      return;
    }
    this._pendingDelete = setTimeout(async () => {
      this._pendingDelete = undefined;
      try {
        await this.statResource();
        this.onResourceEvent();
      } catch {
        this.onResourceMissing();
      }
    }, DELETE_GRACE_PERIOD_MS);
  }

  private onResourceMissing(): void {
    // Keep showing the last version, and reload if the file comes back. Poll
    // for that, as folder watchers stop when the folder itself is removed.
    this._reloadedVersion = undefined;
    this.postToWebview('deleted');
    this._missingPoll = setInterval(
      () => this.onResourceEvent(),
      MISSING_POLL_INTERVAL_MS
    );
  }

  private clearMissingState(): void {
    if (this._pendingDelete) {
      clearTimeout(this._pendingDelete);
      this._pendingDelete = undefined;
    }
    if (this._missingPoll) {
      clearInterval(this._missingPoll);
      this._missingPoll = undefined;
    }
  }

  public print(): void {
    this.postToWebview('print');
  }

  private postToWebview(type: 'reload' | 'deleted' | 'print'): void {
    if (this._previewState !== 'Disposed') {
      this.webviewEditor.webview.postMessage({ type });
    }
  }
  ...
  private getWebviewContents(): string {
    ...
    const settings = {
      ...
      position: this.positions.get(this.resource),
      defaults: {
...
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; connect-src ${cspSource} blob: data:; script-src ${cspSource} 'wasm-unsafe-eval'; worker-src ${cspSource} blob:; style-src 'unsafe-inline' ${cspSource}; img-src blob: data: ${cspSource}; font-src data: ${cspSource}; media-src blob:;">
...
<script src="${resolveAsUri('lib', 'main.mjs')}" type="module"></script>
